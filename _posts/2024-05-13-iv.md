...
If the control variables $$U_i$$ were observed, we would include it in the regression of $$d$$ on $$Y$$, resulting in the following regression equation: $$Y_i = \alpha + \rho d_i + U'_i \gamma + \nu_i$$. In this model, the error term $$\nu_i$$ represents the random component of the potential outcomes after accounting for $$U_i$$. By constructin, this error term is uncorrelated with the treatment $$d_i$$. If our assumption holds, specifically that $$E[d_i \nu_i] = 0$$, the population regression of $$Y_i$$ on $$d_i$$ and $$U_i$$ yields unbiased estimates of the coefficient $$\rho$$. 

The problem we want to tackle is how to estimate the long regression coefficient, $$\rho$$, when $$U_i$$ is unobserved. IV methods can be used to accomplish this when the researcher has access to a variable $$z_i$$ that is correlated with the causal variable of interest, $$d_i$$, but uncorrelated with any other determinants of the dependent variable. 


exclusion restriction - $`Cov(\eta_i, z_i) = 0`$

relevance of instrument - $`Cov(d_i, z_i) \neq 0`$

```math
\beta = \frac{Cov(Y_i, z_i){Var(z_i)}
```

```math
\pi = \frac{Cov(d_i, z_i)}{Var(z_i)}
```


```math
\rho = \frac{\beta}{\pi} = \frac{Cov(y_i, z_i)/V(z_i)}{Cov(d_i, z_i) / V(z_i)} = \frac{Cov(y_i, z_i)}{Cov(d_i, z_i)}
```


Based on the exclusion restriction, the instrument $$z_i$$ affects the outcome $$Y_i$$ only through the treatment $$d_i$$. This implies: $$Cov(\eta_i, z_i) = 0$$. Also, as the instrument $$z_i$$ must be correlated with the treatment $$d_i$$, $$Cov(d_i, z_i) \neq 0$$. 
...
2. First stage equation: The relationship between the treatment $$d_i$$ and the instrument $$z_i$$ is described by the first stage equation: $$\pi = \frac{Cov(d_i, z_i)}{Var(z_i)}$$. Here, $$\pi$$ represents the effect of the instrument $$z_i$$ on the treatment $$d_i$$.

3. IV estimator: The IV estimator for $$\rho$$ is the ratio of these two regression coefficients: $$\rho = \frac{\beta}{\pi} = \frac{Cov(y_i, z_i)/V(z_i)}{Cov(d_i, z_i) / V(z_i)} = \frac{Cov(y_i, z_i)}{Cov(d_i, z_i)}$$.
...
Here, the phrase "uncorrelated with any other determinants of the dependent variables" is like saying $$Cov(\eta_i, z_i) = 0$$, or equivalently, $$z_i$$ is uncorrelated with both $$U_i$$ and $$\nu_i$$. Given the exclusion restriction, it follows that $$\rho = \frac{Cov(Y_i, z_i)}{Cov(d_i, z_i)} = \frac{Cov(y_i, z_i)/V(z_i)}{Cov(d_i, z_i) / V(z_i)}.$$ The second equality is useful because it is usually easier to think in terms of regression coefficients than in terms of covariances. The coefficient of interest, $$\rho$$ is the ratio of the population of regression of $$Y_i$$ on $$z_i$$ (called the reduced form) to the poppulation regression of $$d_i$$ on $$z_i$$ (called the first stage). The IV estimator is the sample analog of the above expression. Note that the IV estimand is predicated on the notion that the first stage is not zero, but this is something you can check in the data. As a rule, if the first stage is only marginally significantly different from zero, the resulting IV estimates are unlikely to be informative, a point we return to later.
