...
## ?
Let's examine the relationship between instrumental variable $$Z$$, a treatment $$D$$, unmeasured confounders $$U$$, and an outcome $$Y$$ through a directed acyclic graph. In this graph, an arrow from the treatment $$D$$ to the outcome $$Y$$ indicates that changing the value of $$D$$ causes a change in $$Y$$. The variable $$U$$ is a source of concern because sit is causally linked to both $$Y$$ and the assignment of $$D$$. Since $$U$$ is not recorded in the data set, it cannot be directly adjusted for using statistical methods such as propensity scores or regression. 
