...
Let's delve into the mechanics of IV. Suppose that the potential outcomes can be expressed as $$Y_{si} = f_i(s)$$, where $$f_i(s) = \alpha + \rho s + \eta_i$$. Imagine there is a vector of control variables $$A_i$$ that account for unobservable characteristics. We can decompose the error term $$\eta_i$$ as $$\eta_i = A_i^' \gamma + \nu_i$$, where $$\gamma$ is a vector of population regression coefficients, and $$\nu_i$$ is a random error term uncorrelated with the control variables $$A_i$$.
