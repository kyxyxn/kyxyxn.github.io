...
Let's delve into the mechanics of IV. Suppose that the potential outcomes can be expressed as $$Y_{si} = f_i(s)$$, where $$f_i(s) = \alpha + \rho s + \eta_i$$. Imagine there is a vector of control variables $$A_i$$ that account for unobservable characteristics. We can decompose the error term $$\eta_i$$ as $$\eta_i = A'_i \gamma + \nu_i$$, where $$\gamma$$ is a vector of population regression coefficients, and $$\nu_i$$ is a random error term uncorrelated with the control variables $$A_i$$.

We assume that the correlation between $$\eta_i$$ and the treatment $$s_i$$ arises solely because of the control variables $$A_i$$. This implies that $$E[s_i\nu_i] = 0$$, meaning there is no corerlation between the treatment $$s_i$$ and the residual error term $$\nu_i$$.

If the control variables $$A_i$$ were observed, we would include it in the regression of $$s$$ on $$Y$$, resulting in the following regression equation: $$Y_i = \alpha + \rho s_i + A'_i \gamma + \nu_i$$. In this model, the error term $$\nu_i$$ represents the random component of the potential outcomes after accounting for $$A_i$$. By constructin, this error term is uncorrelated with the treatment $$s_i$$.

If our assumption holds, specifically that $$E[s_i \nu_i] = 0$$, the population regression of $$Y_i$$ on $$s_i$$ and $$A_i$$ yields unbiased estimates of the coefficient $$\rho$$. 

The problem we want to tackle is how to estimate the long regression coefficient, $$\rho$$, when $$A_i$$ is unobserved. IV methods can be used to accomplish this when the researcher has access to a variable $$z_i$$ that is correlated with the causal variable of interest, $$s_i$$, but uncorrelated with any other determinants of the dependent variable. 

Based on the exclusion restriction, the instrument $$z_i$$ affects the outcome $$Y_i$$ only through the treatment $$s_i$$. This implies: $$Cov(\eta_i, z_i) = 0$$. Also, as the instrument $$z_i$$ must be correlated with the treatment $$s_i$$, $$Cov(s_i, z_i) \neq 0$$. 

The IV approach leverages the properties of the instrument to isolate the exogenous variation in the treatment $$s_i$$ and use it to estimate $$\rho$$. This can be understood through the following steps:

1. Reduced form equation: The relationship between the outcome $$Y_i$$ and the instrument $$z_i$$ is described by: $$\beta = \frac{Cov(Y_i, z_i)}{Var(z_i)}. Here, $$\beta$$ represents the effect of the instrument $$z_i$$ on the outcome $$Y_i$$.

2. First stage equation: The relationship between the treatment $$s_i$$ and the instrument $$z_i$$ is described by the first stage equation: $$\pi = \frac{Cov(s_i, z_i)}{Var(z_i)}$$. Here, $$\pi$$ represents the effect of the instrument $$z_i$$ on the treatment $$s_i$$.

3. IV estimator: The IV estimator for $$\rho$$ is the ratio of these two regression coefficients: $$\rho = \frac{\beta}{\pi} = \frac{Cov(y_i, z_i)/V(z_i)}{Cov(s_i, z_i) / V(z_i)} = \frac{Cov(y_i, z_i){Cov(s_i, z_i)}$$.

This formulation shows that $$\rho$$ can be expressed in terms of covariances because it is derived from the relationship between the instrument and both the outcome and the treatment.



Here, the phrase "uncorrelated with any other determinants of the dependent variables" is like saying $$Cov(\eta_i, z_i) = 0$$, or equivalently, $$z_i$$ is uncorrelated with both $$A_i$$ and $$\nu_i$$. Given the exclusion restriction, it follows that $$\rho = \frac{Cov(Y_i, z_i)}{Cov(s_i, z_i)} = \frac{Cov(y_i, z_i)/V(z_i)}{Cov(s_i, z_i) / V(z_i)}.$$ The second equality is useful because it is usually easier to think in terms of regression coefficients than in terms of covariances. The coefficient of interest, $$\rho$$ is the ratio of the population of regression of $$Y_i$$ on $$z_i$$ (called the reduced form) to the poppulation regression of $$s_i$$ on $$z_i$$ (called the first stage). The IV estimator is the sample analog of the above expression. Note that the IV estimand is predicated on the notion that the first stage is not zero, but this is something you can check in the data. As a rule, if the first stage is only marginally significantly different from zero, the resulting IV estimates are unlikely to be informative, a point we return to later.
