---
layout: single
title: Achieving Balance in Non-Randomized Studies
category: Notes
---

> Will have to add STATA codes and simulation
...
Randomization serves as the most effective way to achieve inter-group comparability and to balance covariate between groups. However, randomization is not always possible due to ethical concerns and high costs.

Propensity score analysis is an effective method for controlling confounders, allowing researchers to balance the backgrounds of patients across various potential risk factors. This method was first introduced by Rosenbaum and Rubin in 1983 (Rosenbaum and Rubin, 1983).
...
Propensity scores are generally estimated using a multivariable logistic regression model, where treatment status is regressed against potential confounders. 

There is some controversy on variable selection for constructing the propensity score model (e.g., whether to include all covariates, those that are associated with the outcome, or with the treatment assignment, or with both.) (Millimet and Tchernis 2009), but including all measured covariates is the simples way that can satisfy ignorable treatment assignment, and mimic a randomized controlled trial by removing all sources of incomparability between the groups (Rubin and Thomas 1996). 
...
### 1:1 or 1:n matching
Propensity score matching involves pairing treated and untreated subjects who have similar propensity scores to form matched sets. Those subjects with overlapping propensity scores across the treated and. Control group can be used, as they are similar enough to allow for matching.
...
For a continuous covariate, the standardized difference is defined as follows:
...
Unlike other hypothesis-testing statistics, the standardized difference is unaffected by sample size. This characteristic makes the standard difference useful for comparing the balance of measured variables between treated and untreated subjects in both matched and unmatched samples. A standardized difference less than 0.1 indicates a negligible difference in the mean or proportion of a covariate between the treatment and control groups (Ho, Imai, King, and Stuart 2007).

Although it is standard practice to report a summary of baseline characteristics across groups (Moher, Schulz, and Altman (2001)), reliance on $p$ values from t-tests or chi-square tests to assess the balance of covariates can be misleading. This is because the failure to reject the null hypothesis does not conclusively indicate a successful balance of covariates between the groups. 
